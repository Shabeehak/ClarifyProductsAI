"""
Gemini Product Extraction Test - Measures accuracy using Gemini AI

Tests OCR + Gemini AI pipeline to extract product names without hardcoded patterns.
Run from backend/tests directory: python test_gemini_extraction.py
"""
...
from app.services.gemini_product_extractor import get_gemini_extractor

# Setup loguru logging
log_dir = Path(__file__).parent / "logs"
log_dir.mkdir(exist_ok=True)

log_file = log_dir / f"gemini_extraction_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def test_gemini_extraction():
    """Test Gemini extraction on sample OCR outputs from log file."""
    logger.info("="*80)
    logger.info("Gemini Product Extraction Test Started")
    ...
    logger.info("Initializing Gemini Product Extractor")
    try:
    ...
    logger.info(f"\nTesting Gemini extraction on {len(test_cases)} OCR failure cases")
    logger.info("="*80)
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        extracted = extractor.extract_product_name(ocr_output)

        if extracted:
            logger.info(f"Gemini:      {extracted}")

            # Check if Gemini extraction matches expected
            ...
            logger.error("FAILED (Gemini extraction returned None)")
            results.append({
                "source": source,
                "expected": expected,
                "ocr": ocr_output,
    ...
    logger.info(f"\nBefore Gemini (Raw OCR):    {correct_before}/{len(test_cases)} correct ({before_accuracy:.1f}%)")
    logger.info(f"After Gemini (OCR + AI):    {correct_after}/{len(test_cases)} correct ({after_accuracy:.1f}%)")
    ...
    for result in results:
        ...
        logger.info(f"  Gemini:    {result['extracted']}")
        if result['status'] in ['CORRECT', 'WRONG']:
            logger.info(f"  Match:     {result['match_ratio']:.1%}")

    logger.info("\n" + "="*80)
    logger.info("Gemini Extraction Test Completed")
    logger.info("="*80)
...
def main():
    """Run Gemini extraction test."""
    try:
        accuracy = test_gemini_extraction()

        print("\n" + "="*80)
        print("SUMMARY")
        print("="*80)
        print(f"Gemini AI Accuracy: {accuracy:.1f}%")
        ...
        print("="*80)
        print("Approach              | Accuracy | Pros                      | Cons")
        print("-"*80)
        print("GLiNER NER           | 0%       | Fast, offline             | Not trained for products")
        print("Text Cleaning        | 25%      | Fast, no API              | Hardcoded patterns")
        print("Gemini AI            | {:.1f}%    | No hardcoding, context    | API calls, cost".format(accuracy))
